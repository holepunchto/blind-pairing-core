...
const additionalNodes = [
  ...
  { host: '8.8.8.8', port: 8081 },
  { host: '13.160.0.1', port: 8081 },
  { host: '127.0.0.1', port: 8082 },
  { host: '192.168.0.1', port: 8080 },
  { host: '224.0.1.1', port: 8082 }
]

const checkAdditionalNodes = (t, invite) => {
  const { additionalNodes } = decodeInvite(invite)

  t.alike(additionalNodes, [
    { host: '8.8.8.8', family: 4, port: 8080 },
    { host: '8.8.8.8', family: 4, port: 8081 },
    { host: '13.160.0.1', family: 4, port: 8081 }
  ])
}

test('invite - bogon', async (t) => {
  t.plan(1)
...
test('invite - bad nodes', async (t) => {
  t.plan(1)

  const key = b4a.allocUnsafe(32).fill(1)

  const { invite } = createInvite(key, {
    additionalNodes: { host: '127.0.0.1', port: 8082 }
  })
  const { additionalNodes } = decodeInvite(invite)

  t.alike(additionalNodes, null)
})

test('invite - no nodes', async (t) => {
  t.plan(1)
...
test('basic valid pairing', async (t) => {
  ...
  const { invite, publicKey } = createInvite(key, { additionalNodes })

  checkAdditionalNodes(t, invite)
...
test('basic valid pairing with encryption key and data', async (t) => {
  ...
  const { invite, publicKey, additional } = createInvite(key, {
    data,
    additionalNodes
  })
...
test('does not leak invitee key to unproven inviters', async (t) => {
  ...
  const key = b4a.allocUnsafe(32).fill(1)
  const { invite } = createInvite(key, { additionalNodes })
...
test('invite response is static', async (t) => {
  ...
  const invite = createInvite(key, { additionalNodes })
  checkAdditionalNodes(t, invite.invite)

  const invite2 = createInvite(key, { additionalNodes })
  checkAdditionalNodes(t, invite2.invite)
