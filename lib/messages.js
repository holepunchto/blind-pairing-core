...
const ResponseError = c.uint

const ResponsePayload = {
  preencode (state, p) {
    state.end++ // flags
    if (p.error) {
      ResponseError.preencode(state, p.error)
      return
    }
  ...
  encode (state, p) {
    if (p.error) {
      c.uint.encode(state, 0b1)
      ResponseError.encode(state, p.error)
      return
    }

    c.uint.encode(state, p.encryptionKey ? 0b10 : 0)
  ...
  decode (state) {
    ...
    if (flags & 0b1) {
      return {
        error: ResponseError.decode(state),
        key: null,
        encryptionKey: null
      }
    }
    ...
    const encryptionKey = flags & 0b10 ? c.fixed32.decode(state) : null
...
const PersistedRequest = {
  preencode (state, req) {
    ...
    if (req.response) {
      ...
      ResponsePayload.preencode(state, req.response)
    } else {
  ...
  encode (state, req) {
    ...
    if (req.key) {
      ...
      ResponsePayload.encode(state, req.key)
    } else {
  ...
  decode (state, req) {
    ...
    const response = c.uint8.decode(state)
      ? ResponsePayload.decode(state)
      : null
