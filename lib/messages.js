const { isBogon } = require('bogon')
const c = require('compact-encoding')

const DHTNodes = c.array(c.ipv4Address)

const Invite = {
  preencode(state, i) {
    state.end++ // version
    state.end++ // flags
    c.fixed32.preencode(state, i.seed)
    if (i.discoveryKey) c.fixed32.preencode(state, i.discoveryKey)
    if (i.expires) c.uint32.preencode(state, Math.floor(i.expires / 1000)) // store as secs
    if (i.additionalNodes && Array.isArray(i.additionalNodes))
      DHTNodes.preencode(
        state,
        i.additionalNodes.filter((node) => !isBogon(node.host))
      )
  },
  encode(state, i) {
    c.uint.encode(state, 1) // version
    c.uint.encode(
      state,
      (i.discoveryKey ? 1 : 0) |
        (i.expires ? 2 : 0) |
        (i.sensitive ? 4 : 0) |
        (i.testInvitation ? 8 : 0) |
        (i.additionalNodes && Array.isArray(i.additionalNodes) ? 16 : 0)
    )
    c.fixed32.encode(state, i.seed)
    if (i.discoveryKey) c.fixed32.encode(state, i.discoveryKey)
    if (i.expires) c.uint32.encode(state, Math.floor(i.expires / 1000))
    if (i.additionalNodes && Array.isArray(i.additionalNodes))
      DHTNodes.encode(
        state,
        i.additionalNodes.filter((node) => !isBogon(node.host))
      )
  },
  decode(state) {
    const version = c.uint.decode(state)
    if (version !== 1) {
      throw new Error('Unknown invite version')
    }

    const flags = c.uint.decode(state)

    return {
      seed: c.fixed32.decode(state),
      discoveryKey: flags & 1 ? c.fixed32.decode(state) : null,
      expires: flags & 2 ? c.uint32.decode(state) * 1000 : 0,
      sensitive: (flags & 4) !== 0,
      testInvitation: (flags & 8) !== 0,
      additionalNodes: flags & 16 ? DHTNodes.decode(state) : null
    }
  }
}

const RequestPayload = {
  preencode(state, p) {
    c.optionalBuffer.preencode(state, p.session)
    c.optionalBuffer.preencode(state, p.data)
  },
  encode(state, p) {
    c.optionalBuffer.encode(state, p.session)
    c.optionalBuffer.encode(state, p.data)
  },
  decode(state) {
    return {
      session: c.optionalBuffer.decode(state),
      data: c.optionalBuffer.decode(state)
    }
  }
}

const ResponseStatus = c.uint

const AdditionalData = {
  preencode(state, m) {
    c.optionalBuffer.preencode(state, m.data)
    c.fixed64.preencode(state, m.signature)
  },
  encode(state, m) {
    c.optionalBuffer.encode(state, m.data)
    c.fixed64.encode(state, m.signature)
  },
  decode(state) {
    return {
      data: c.optionalBuffer.decode(state),
      signature: c.fixed64.decode(state)
    }
  }
}

const ResponsePayload = {
  preencode(state, p) {
    ResponseStatus.preencode(state, p.status)
    if (p.status !== 0) return

    let flags = 0
    if (p.encryptionKey) flags |= 1
    if (p.additional) flags |= 2
    c.uint.preencode(state, flags) // flags

    c.fixed32.preencode(state, p.key)
    if (p.encryptionKey) c.fixed32.preencode(state, p.encryptionKey)
    if (p.additional) AdditionalData.preencode(state, p.additional)
  },
  encode(state, p) {
    ResponseStatus.encode(state, p.status)
    if (p.status !== 0) return

    let flags = 0
    if (p.encryptionKey) flags |= 1
    if (p.additional) flags |= 2
    c.uint.encode(state, flags)

    c.fixed32.encode(state, p.key)
    if (p.encryptionKey) c.fixed32.encode(state, p.encryptionKey)
    if (p.additional) AdditionalData.encode(state, p.additional)
  },
  decode(state) {
    const status = ResponseStatus.decode(state)

    if (status !== 0) {
      return {
        status,
        key: null,
        encryptionKey: null,
        additional: null
      }
    }

    const flags = c.uint.decode(state)
    const key = c.fixed32.decode(state)
    const encryptionKey = (flags & 1) !== 0 ? c.fixed32.decode(state) : null
    const additional = (flags & 2) !== 0 ? AdditionalData.decode(state) : null

    return {
      status: 0,
      key,
      encryptionKey,
      additional
    }
  }
}

const InviteRequest = {
  preencode(state, i) {
    c.fixed32.preencode(state, i.inviteId)
    RequestPayload.preencode(state, i.payload)
  },
  encode(state, i) {
    c.fixed32.encode(state, i.inviteId)
    RequestPayload.encode(state, i.payload)
  },
  decode(state) {
    return {
      inviteId: c.fixed32.decode(state),
      payload: RequestPayload.decode(state)
    }
  }
}

const InviteResponse = {
  preencode(state, i) {
    c.fixed32.preencode(state, i.id)
    c.optionalBuffer.preencode(state, i.payload)
  },
  encode(state, i) {
    c.fixed32.encode(state, i.id)
    c.optionalBuffer.encode(state, i.payload)
  },
  decode(state) {
    return {
      id: c.fixed32.decode(state),
      payload: c.optionalBuffer.decode(state)
    }
  }
}

const InviteData = {
  preencode(state, i) {
    c.fixed64.preencode(state, i.signature)
    c.optionalBuffer.preencode(state, i.userData)
  },
  encode(state, i) {
    c.fixed64.encode(state, i.signature)
    c.optionalBuffer.encode(state, i.userData)
  },
  decode(state) {
    return {
      signature: c.fixed64.decode(state),
      userData: c.optionalBuffer.decode(state)
    }
  }
}

const InviteReceipt = {
  preencode(state, i) {
    c.fixed32.preencode(state, i.session)
    c.fixed64.preencode(state, i.signature)
    c.optionalBuffer.preencode(state, i.userData)
  },
  encode(state, i) {
    c.fixed32.encode(state, i.session)
    c.fixed64.encode(state, i.signature)
    c.optionalBuffer.encode(state, i.userData)
  },
  decode(state) {
    return {
      session: c.fixed32.decode(state),
      signature: c.fixed64.decode(state),
      userData: c.optionalBuffer.decode(state)
    }
  }
}

const AuthData = {
  preencode(state, i) {
    c.optionalBuffer.preencode(state, i.session)
    c.optionalBuffer.preencode(state, i.userData)
  },
  encode(state, i) {
    c.optionalBuffer.encode(state, i.session)
    c.optionalBuffer.encode(state, i.userData)
  },
  decode(state) {
    return {
      session: c.optionalBuffer.decode(state),
      userData: c.optionalBuffer.decode(state)
    }
  }
}

module.exports = {
  Invite,
  ResponsePayload,
  InviteRequest,
  InviteResponse,
  InviteData,
  InviteReceipt,
  AuthData
}
